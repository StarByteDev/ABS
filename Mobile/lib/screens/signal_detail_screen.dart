...
class _SignalDetailScreenState extends State<SignalDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final values = await Future.wait([
        ...
        api.get(
          '/pulse/execution/ticket',
          query: {'signal_id': widget.signalId},
        ),
        api
            .get('/pulse/signals/${widget.signalId}/validation')
            .catchError((_) => <String, dynamic>{}),
      ]);
      signal = JsonTools.map(
        JsonTools.at(values[0], 'data', <String, dynamic>{}),
      );
      ticketData = JsonTools.map(
        JsonTools.at(values[1], 'data', <String, dynamic>{}),
      );
      validation = JsonTools.map(
  ...
  Widget build(BuildContext context) {
    return AbsPage(
      title: 'Trade Review',
      subtitle:
          'Signal #${widget.signalId} · server-authoritative execution checks',
  ...
  List<Widget> _content() {
    ...
    final direction = JsonTools.text(
      detail['direction'],
      JsonTools.text(signal['direction']),
    ).toUpperCase();
    final environment = JsonTools.text(
      settings['environment'],
      'testnet',
    ).toUpperCase();
    final validationStatus = JsonTools.text(
      validation['status'],
      JsonTools.text(validation['outcome'], 'waiting_entry'),
    );
    final symbol = JsonTools.text(
      detail['symbol'],
      JsonTools.text(signal['symbol']),
    );
    ...
    return [
      PremiumHeroCard(
        ...
        trailing: Container(
          ...
          child: Column(
            ...
            children: [
              Text(
                number(detail['score'] ?? signal['score'], digits: 0),
                style: TextStyle(
          ...
          children: [
            StatusChip(
              ...
              good: direction == 'LONG'
                  ...
                  ? false
                  : null,
              warning: direction != 'LONG' && direction != 'SHORT',
            ),
            StatusChip(
              environment == 'LIVE' ? 'LIVE ACCOUNT' : 'TESTNET',
              warning: environment == 'LIVE',
            ),
        ...
        children: [
          Expanded(
            child: OutlinedButton.icon(
              onPressed: sharing ? null : _shareSignal,
              icon: sharing
                  ? const SizedBox(
                      width: 17,
                      height: 17,
              ...
              label: Text(sharing ? 'Preparing...' : 'Share Signal'),
            ),
          ),
          const SizedBox(width: 10),
          Expanded(
            child: OutlinedButton.icon(
              onPressed: explaining ? null : _explainSignal,
              icon: explaining
                  ? const SizedBox(
                      width: 17,
                      height: 17,
              ...
              label: Text(explaining ? 'Explaining...' : 'AI Explain'),
            ),
          ),
        ],
      ),
        ...
        child: Column(
          ...
          children: [
            const AbsSectionTitle(
              'Trade plan',
              subtitle: 'The values that define this setup.',
            ),
            const SizedBox(height: 10),
            Row(
              children: [
                Expanded(
                  child: _TradePlanMetric(
                    label: 'Current ABS price',
                    value: money(detail['last_price'] ?? signal['last_price']),
                  ),
                ),
            ...
            KeyValueRow(
              'Order type',
              JsonTools.text(ticket['order_type'], 'LIMIT'),
            ),
            KeyValueRow('Quantity', number(ticket['quantity'], digits: 6)),
            KeyValueRow(
              'Leverage',
              '${JsonTools.integer(ticket['leverage'], 1)}x',
            ),
            KeyValueRow(
              'Validation state',
              validationStatus.replaceAll('_', ' ').toUpperCase(),
            ),
          ],
        ),
      ),
        ...
        AbsCard(
          ...
          child: const Row(
            ...
            children: [
              ...
              Expanded(
                child: Text(
                  'Before you confirm: Entry is where ABS intends the trade to open. Take Profit is the planned profit exit. Stop Loss is the protection exit. Live orders can use real funds, so review all three before execution.',
      ...
      const AbsSectionTitle(
        ...
        subtitle: 'Every required check must pass on the ABS server.',
      ),
      const SizedBox(height: 10),
      AbsCard(
        child: Column(
          children: checks.isEmpty
              ? [
                  const Text(
                    'Execution checks are unavailable.',
      ...
      AbsSectionTitle(
        'Risk & margin',
        subtitle: session.proMode
            ? 'Full server-calculated account context.'
            : 'The most important risk numbers first.',
      ),
      const SizedBox(height: 10),
      AbsCard(
        child: Column(
          children: [
            KeyValueRow(
              'Estimated risk',
              money(calc['estimated_risk']),
              valueColor: AbsColors.gold,
            ),
            KeyValueRow(
              'Potential reward',
              money(calc['potential_reward']),
              valueColor: AbsColors.green,
            ),
            KeyValueRow(
              'Risk / reward',
              number(calc['risk_reward'], digits: 2),
            ),
            ...
            if (session.proMode) ...[
              ...
              KeyValueRow(
                'Current exposure',
                percent(account['current_exposure']),
              ),
            ] else
              ExpansionTile(
        ...
        const AbsCard(
          ...
          child: Row(
            ...
            children: [
              ...
              Expanded(
                child: Text(
                  'LIVE account selected. Confirming can place a real Binance Futures order. Review entry, quantity, leverage, TP and SL carefully.',
      ...
      SizedBox(
        ...
        child: ElevatedButton.icon(
          onPressed: ready && !executing
              ? () =>
                    _confirmAndExecute(ticket, settings, direction, environment)
              : null,
          icon: executing
              ? const SizedBox(
                  width: 18,
          ...
          label: Text(
            executing
                ...
                : 'Resolve blocked checks first',
          ),
        ),
      ),
      const SizedBox(height: 10),
      const Text(
        'ABS does not treat a submitted order as an open position until Binance confirms the entry fill and protection state.',
  ...
  Future<void> _shareSignal() async {
    ...
    try {
      final response = await SessionScope.of(context).api.post(
        '/pulse/signals/${widget.signalId}/share',
        body: {'channel': 'mobile_share_sheet'},
      );
      final data = JsonTools.map(
        JsonTools.at(response, 'data', <String, dynamic>{}),
      );
      final text = JsonTools.text(data['text'], '');
      if (text.isEmpty)
        throw const ApiException('ABS could not prepare the share card.');
  ...
  Future<void> _explainSignal() async {
    setState(() => explaining = true);
    try {
      final response = await SessionScope.of(context).api
          .post('/pulse/signals/${widget.signalId}/explain');
      final explanation = JsonTools.text(
        JsonTools.at(response, 'data.explanation'),
        '',
      );
      if (explanation.isEmpty)
        throw const ApiException('ABS returned an empty explanation.');
      if (!mounted) return;
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    explanation,
                    style: const TextStyle(color: AbsColors.text, height: 1.55),
                  ),
                  const SizedBox(height: 16),
                  const Text(
                    'Educational explanation only. Independently assess market and account risk before acting.',
                    style: TextStyle(color: AbsColors.muted, fontSize: 10.5),
                  ),
                ],
              ),
  ...
  Future<void> _confirmAndExecute(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: Text(
              environment == 'LIVE' ? 'Execute LIVE' : 'Execute Testnet',
            ),
          ),
        ],
      ),
    ...
    try {
      final body = <String, dynamic>{
        ...
        'order_type': JsonTools.text(
          ticket['order_type'],
          'LIMIT',
        ).toUpperCase(),
        'leverage': JsonTools.integer(ticket['leverage'], 1),
        'quantity': JsonTools.number(ticket['quantity']) > 0
            ? JsonTools.number(ticket['quantity'])
            : null,
        'notional': null,
        'price': JsonTools.number(ticket['limit_price']) > 0
            ? JsonTools.number(ticket['limit_price'])
            : null,
        'stop_loss': JsonTools.number(ticket['stop_loss']),
        'take_profit': JsonTools.number(ticket['take_profit']),
        'position_side': JsonTools.text(
          settings['position_mode'],
          'BOTH',
        ).toUpperCase(),
        'time_in_force': JsonTools.text(
      ...
      final response = await SessionScope.of(context).api
          .post('/pulse/signals/${widget.signalId}/execute', body: body);
      if (!mounted) return;
      showSnack(
        context,
        JsonTools.text(
    ...
    child: Column(
      ...
      children: [
        ...
        Text(
          ...
          style: TextStyle(
            color: valueColor ?? AbsColors.text,
            fontSize: 13,
            fontWeight: FontWeight.w900,
          ),
        ),
