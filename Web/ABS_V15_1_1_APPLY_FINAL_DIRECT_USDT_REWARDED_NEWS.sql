-- Alpha Block Solutions V15.1.1
-- Direct USDT package activation + anonymous rewarded free-signal access + ABS News macro intelligence.
...
-- ABS V15.1.1 — economic calendar / ABS News macro intelligence extension.
SET @q = IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='economic_events' AND COLUMN_NAME='provider_event_id')=0,
  'ALTER TABLE `economic_events` ADD COLUMN `provider_event_id` VARCHAR(190) NULL, ADD INDEX `economic_events_provider_event_id_index` (`provider_event_id`)', 'SELECT 1');
PREPARE s FROM @q; EXECUTE s; DEALLOCATE PREPARE s;
SET @q = IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='economic_events' AND COLUMN_NAME='source_url')=0,
  'ALTER TABLE `economic_events` ADD COLUMN `source_url` VARCHAR(500) NULL', 'SELECT 1');
PREPARE s FROM @q; EXECUTE s; DEALLOCATE PREPARE s;
SET @q = IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='economic_events' AND COLUMN_NAME='crypto_impact')=0,
  'ALTER TABLE `economic_events` ADD COLUMN `crypto_impact` VARCHAR(20) NULL, ADD INDEX `economic_events_crypto_impact_index` (`crypto_impact`)', 'SELECT 1');
PREPARE s FROM @q; EXECUTE s; DEALLOCATE PREPARE s;
SET @q = IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='economic_events' AND COLUMN_NAME='easy_explanation')=0,
  'ALTER TABLE `economic_events` ADD COLUMN `easy_explanation` TEXT NULL', 'SELECT 1');
PREPARE s FROM @q; EXECUTE s; DEALLOCATE PREPARE s;
SET @q = IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='economic_events' AND COLUMN_NAME='crypto_impact_summary')=0,
  'ALTER TABLE `economic_events` ADD COLUMN `crypto_impact_summary` TEXT NULL', 'SELECT 1');
PREPARE s FROM @q; EXECUTE s; DEALLOCATE PREPARE s;
SET @q = IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='economic_events' AND COLUMN_NAME='is_crypto_relevant')=0,
  'ALTER TABLE `economic_events` ADD COLUMN `is_crypto_relevant` TINYINT(1) NOT NULL DEFAULT 1, ADD INDEX `economic_events_is_crypto_relevant_index` (`is_crypto_relevant`)', 'SELECT 1');
PREPARE s FROM @q; EXECUTE s; DEALLOCATE PREPARE s;
SET @q = IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='economic_events' AND COLUMN_NAME='synced_at')=0,
  'ALTER TABLE `economic_events` ADD COLUMN `synced_at` TIMESTAMP NULL, ADD INDEX `economic_events_synced_at_index` (`synced_at`)', 'SELECT 1');
PREPARE s FROM @q; EXECUTE s; DEALLOCATE PREPARE s;

INSERT INTO `site_settings` (`key`,`value`,`type`,`group`) VALUES
('economic_calendar_auto_sync','1','boolean','integrations')
