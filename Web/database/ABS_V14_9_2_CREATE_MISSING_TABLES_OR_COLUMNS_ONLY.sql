...
-- 3) It creates missing tables and adds missing columns used by ABS V14.9.2.
...
-- ABS V14.9.2 administrator event-notification defaults.
-- These INSERTs are idempotent and DO NOT overwrite existing administrator choices.
INSERT INTO `site_settings` (`key`, `value`, `type`, `group`)
SELECT 'admin_notification_email', 'i@armansabir.com', 'string', 'communications'
WHERE NOT EXISTS (SELECT 1 FROM `site_settings` WHERE `key`='admin_notification_email');

INSERT INTO `site_settings` (`key`, `value`, `type`, `group`)
SELECT 'admin_notify_new_registration', '1', 'boolean', 'communications'
WHERE NOT EXISTS (SELECT 1 FROM `site_settings` WHERE `key`='admin_notify_new_registration');

INSERT INTO `site_settings` (`key`, `value`, `type`, `group`)
SELECT 'admin_notify_new_subscription', '1', 'boolean', 'communications'
WHERE NOT EXISTS (SELECT 1 FROM `site_settings` WHERE `key`='admin_notify_new_subscription');
...
SELECT 'ABS V14.9.2 schema create/repair completed. missing_required_columns should be 0.' AS result;