...
class SettingsController extends Controller
{
    public function edit(Request $request, PulseAccessService $access, PulsePairAccessService $pairAccess, PulsePairSelectionLockService $pairLock, PulseMarketDataService $market)
    {
        ...
        $selectedCount = count(array_filter((array) ($settings->selected_pairs ?? [])));
        $setupSteps = [
            ...
            ['key' => 'pairs', 'label' => 'Markets', 'ready' => $selectedCount > 0, 'href' => '#selected-markets'],
        ...
        return view('pulse.settings', [
            ...
            'pairCatalog' => $pairAccess->catalog($user, $settings->selected_pairs ?? []),
            'pairSelectionLock' => $pairLock->status($settings),
            'plan' => $user->pulsePlan(),
    ...
    public function update(Request $request, PulseAuditService $audit, PulseAccessService $access, PulsePairAccessService $pairAccess, PulsePairSelectionLockService $pairLock)
    {
        ...
        $data = $request->validate([
            ...
            'selected_pairs.*' => ['string', 'max:30', 'exists:pulse_pairs,symbol'],
        ]);

        $maxPairs = max(1, (int) ($plan?->max_selected_pairs ?: 5));
        $selected = array_values(array_unique(array_map('strtoupper', $data['selected_pairs'] ?? [])));
        if (count($selected) > $maxPairs) {
            return back()->withErrors(['selected_pairs' => "Your Pulse plan allows up to {$maxPairs} selected pairs."])->withInput();
        }
        $allowedSymbols = $pairAccess->allowedSymbols($request->user());
        $notAllowed = array_values(array_diff($selected, $allowedSymbols));
        if ($notAllowed !== []) {
            return back()->withErrors(['selected_pairs' => 'One or more selected markets are not included in your current Pulse plan.'])->withInput();
        }
        ...
        try {
            ...
            return back()->withErrors(['selected_pairs' => $e->getMessage()])->withInput();
        }

        // Signal-only users can configure scanner/risk preferences without exchange execution.
        // Environment eligibility is enforced only when an execution mode can submit orders.
        ...
        $audit->record('settings.updated', $request->user(), 'PulseUserSetting', $settings->id, $settings->environment, [
            'execution_mode' => $settings->execution_mode, 'selected_pairs' => $selected,
            'emergency_stop' => $settings->emergency_stop,
            'pair_selection_locked_until' => $settings->pair_selection_locked_until?->toIso8601String(),
        ], $request);

        return back()->with('success', 'Pulse preferences were saved. Guided signal execution continues to use ABS-managed defaults and your risk limits.');
    }
